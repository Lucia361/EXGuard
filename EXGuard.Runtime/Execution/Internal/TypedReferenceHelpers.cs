...
namespace EXGuard.Runtime.Execution.Internal {
	internal static unsafe class TypedReferenceHelpers {
		...
		static Cast BuildCastHelper(Type sourceType, Type targetType) {
			var dm = new DynamicMethod("", typeof(void), new[] { typeof(TypedRefPtr) }, typeof(TypedReferenceHelpers).Module, true);
		...
		static Make BuildMakeHelper(Type targetType) {
			var dm = new DynamicMethod("", typeof(void), new[] { typeof(void*), typeof(TypedRefPtr) }, typeof(TypedReferenceHelpers).Module, true);
			var gen = dm.GetILGenerator();
		...
		static Unbox BuildUnboxHelper(Type boxType) {
			var dm = new DynamicMethod("", typeof(void), new[] { typeof(object), typeof(TypedRefPtr) }, typeof(TypedReferenceHelpers).Module, true);
			var gen = dm.GetILGenerator();
		...
		static FieldAdr BuildAddrHelper(FieldInfo field) {
			var dm = new DynamicMethod("", typeof(void), new[] { typeof(TypedRefPtr), typeof(TypedRefPtr) }, typeof(TypedReferenceHelpers).Module, true);
			var gen = dm.GetILGenerator();
